...
export class ClientsComponent {
  ...
  public data$ = combineLatest([this.conditions$, this.refresh$]).pipe(
    switchMap(([conditions]) => this.clientHttpService.list({ tenantId: TenantService.tenantId, ...toListRequest<Client>(conditions, undefined, ['uuid', 'name']) })),
    shareReplayOne(),
  );

  public openClientDialog(uuid?: string): void {
    this.dialogService
      .open(ClientDialogComponent, { header: this.translateService.instant('simple.client'), width: '540px', modal: true, closable: true, data: uuid })
